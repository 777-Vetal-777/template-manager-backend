...
public interface WorkspaceController {
    ...
    @PutMapping
    @PreAuthorize("hasAnyAuthority('E4_US18_WORKSPACE_DEFAULT_SETTINGS', 'E4_US23_WORKSPACE_SET_UP_WIZARD')")
    @Operation(summary = "Specify a Stage Instance.", description = "Specify which instance will be the default on stage. ",
            security = @SecurityRequirement(name = OpenApiConfig.BEARER_AUTH_SECURITY_SCHEME_NAME))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Instance is successfully linked to the stage ", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = WorkspaceDTO.class))}),
            @ApiResponse(responseCode = "400", description = "Invalid input", content = @Content),
    })
